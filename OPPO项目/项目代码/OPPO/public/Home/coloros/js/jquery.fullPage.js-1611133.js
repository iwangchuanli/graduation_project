...
(function (global, factory) {
    ...
    $.fn.fullpage = function (options) {
        ...
        function moveSlide(direction, section) {
            var activeSection = section ? $(section) : $(SECTION_ACTIVE_SEL);
        ...
        function performMovement(v) {
            ...
            var windowsHeight = $window.height();
            var preInnerWrapperHeight = $("#section_" + v.sectionIndex).height();
            var need_top = preInnerWrapperHeight - windowsHeight; //最后一屏被遮盖的高度
            if (need_top <= 0) {
                need_top = 0;
            }
            if (options.css3 && options.autoScrolling && !options.scrollBar) {
                if (v.sectionIndex == options.navigationTooltips.length - 1) {
                    if (!ismobile) {
                        v.dtop = Math.abs($("#fullpage").offset().top - 215 - need_top);
                    }
                    else {
                        // 虚拟按键
                        if (window.navigator.userAgent.toLowerCase().indexOf("r7") != -1) {
                            need_top += 35;
                        }
                        v.dtop = Math.abs($("#fullpage").offset().top - 80 - need_top);
                    }
                }
