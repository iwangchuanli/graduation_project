  ...
  <li>
    ...
    <div class="step-content">
      ...
      <div class="step-gradient"></div>
		...
		<form action="/fillOrder"  method="get">
              ...
              <ul class="purchase-list-header">
                <li class="first">
                  <label class="check-box-label check-box-all" for="checkAll">
                    <span class="icon icon-checkbox allCheck"></span>
			   ...
			   <input type="hidden" id="svn" name="svn" value="">
              <div class="brick-shadow purchase-list" id="del">
                <div class="cart-product cart-product-l cart-product-10da7c9a79119688c135223cd07efc0c ">
					<div class="cart-product-choice">
										<label id="oneCheck" data-id="{{$carts['cid']}}" class="check-box-label check-box-item">
										
							<span class="icon icon-checkbox oneCheck" price="{{$carts['gprice']}}" num="{{$carts['num']}}"></span>
						</label>
				...
				<div class="cart-product-info">
					...
					<div class="cart-counter-box">
						<div class="counter-box">
							<a class="btn minus" id="minus" data-id="{{$carts['cid']}}">-</a>
							<input class="number" id="number" readonly="" type="text" value="{{$carts['num']}}" data-quantity="1" data-price="{{$carts['gprice']}}">
							<a class="btn plus" id="plus" data-id="{{$carts['cid']}}">+</a>
						</div>
					</div>
					<div class="cart-product-price">
						<span class="normals" id="normals">{{$carts['gprice']*$carts['num']}}</span>
					</div>
            ...
            <span class="cart-fee-title"><i class="icon-hb"></i>订单满600可使用花呗分期3/6/12期</span>
              ...
              <div class="form-actions brick-shadow">
                <div class="form-actions-left">
                  <a class="link-primary" href="/shop/index" target="_blank">继续购买&gt;&gt;</a>
                </div>
                <div class="form-actions-right">
                    <button type="submit" class="button cart-button" >结算</button>
                </div>
              </div>
            </div>
		</form>
...
<script>
	...
	$(function(){
		$('a[id="plus"]').click(function(){
			...
			$.ajax({
				...
				type:'GET',
				success:function(data){
					$th.prev().val(data.num);
					$th.parent().parent().next().first().html(data.gprice*data.num);
	...
	$(function(){
		...
		var allnum=0;//总数量 
		 ...
		 var cid = '';
		  $('label[id="oneCheck"]').bind('click',function(){
				$(this).toggleClass('xdd');
				if($(this).hasClass('xdd')){
					var data = $(this).attr('data-id');
					cid+=data+',';
				}else{
					cid = '';
				}
				$('input[id="svn"]').val(cid);
		  });
