          	        ...
          	        <div class="select-territories" >
						...
						<div class='basic-input one-whole desk-one-fifth inline-block'>
							...
							<select id='select' name='province_id' class="sheng">
								@foreach($sheng as $she)
									<option name="{{$she}}">{{$she}}</option>
								@endforeach
							</select>
						</div>
						@foreach($city as $one=>$two)
							<div class='basic-input one-whole desk-one-fifth inline-block' name="xuan" id="{{$one}}" style="display:none">
								<span class='icon icon-grey-arrow-down'></span>
								<select id="select" name="city_id" class="shi {{$one}}">
									@foreach($two as $three=>$four)
									<option id="{{$one}}" class="{{$four}}">{{$four}}</option>
									@endforeach
								</select>
							</div>
						@endforeach
						<div class='basic-input one-whole desk-one-fifth inline-block city-select'>	
							<form action="/service/map" method="get">
								<input type="hidden" name="ssheng" value="{{$ssheng}}"></input>
								<input type="hidden" name="sshi" value="{{$sshi}}"></input>
							...
							@foreach($result as $key=>$one)
		          			<li class='store-location js-collapse'>
								...
								<div class='gi desk-one-fifth'>
									<p>工作时间:09:00-17:00(休息时间:{{$one['sxiu']}})</p>
								</div>
								<div class='gi desk-one-fifth'>
									<p>{{$one['snumber']}}<br>（{{$one['sshi']}}统一咨询电话）</p>
								</div>
...
<script src="{{asset('/Home/oppozhu/js/jquery-1.7.2.js')}}"></script>
			<input type="hidden" name="" class="a" value="{{$address}}">
...
<script src="{{asset('Home/oppozhu/js/scripts.js-v=201506151120596.js')}}" ></script>
  <script src="{{asset('Home/oppozhu/js/region_new.js')}}" ></script>
