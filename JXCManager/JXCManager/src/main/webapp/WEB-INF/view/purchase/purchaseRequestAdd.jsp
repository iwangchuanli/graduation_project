                                ...
                                <table style="width: 80%; margin: 0px auto;">
                                    <tr>
                                        <td>
                                            <div class="control-group">
                                                ...
                                                <div class="controls">
                                                    <input type="hidden" name="purchaseRequestId" value="${orderNo}"/>
                                                    <span class="input-xlarge uneditable-input">${orderNo }</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="control-group">
                                                <label class="control-label"
                                                       for="supplier.supplierId">供应商&nbsp;&nbsp;</label>
                                                <div class="controls">
                                            ...
                                            <div class="control-group">
                                                ...
                                                <div class="controls">
                                                    <input class="input-xlarge focused" id="suppContact"
                                                           maxlength="8" name="suppContact" type="text"
                                                           placeholder="此处填写供货商的联系人"/>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="control-group">
                                                ...
                                                <div class="controls">
                                                    <input class="input-xlarge focused" id="suppPhone"
                                                           maxlength="11" name="suppPhone" type="text"
                                                           placeholder="此处填写联系人的电话"/>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
            ...
            <table class="table table-bordered" style="table-layout: fixed;"
                ...
                <c:forEach items="${productAll.list}" var="s">
                    <tr height="20px;">
                        ...
                        <td><input type="number" min="0" value="1"
            ...
            <div class="pagination pagination-centered">
                <ul id="productPageButton">
                    <li><a href="javascript:;"
                           onclick="javascript:goproductpage('pre');">上一页</a> <input
                            type="hidden" name="productPageNow" value="${productAll.pageNum}"/>
                    </li>
                    <c:forEach begin="1" end="${productAll.pages}" var="s">
                        <li><a href="javascript:;" onclick="javascript:goproductpage(${s});">${s}</a></li>
                    </c:forEach>
                    <li><a href="javascript:;"
                           onclick="javascript:goproductpage('next');">下一页</a></li>
                </ul>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    $(function () {
        whetherPro();
        //设置下拉列表为只读
        $("#employeeByRequestEmpId.empLoginName").attr("readonly", "readonly");
        //设置时间控件默认为当前时间
        var date = new Date();
        var dateNow = (date.getMonth() + 1) + "/" + date.getDate() + "/" + date.getFullYear();
        $("#requestTime").val(dateNow);
        $("#purchaseRequestform").submit(function () {
            var tbody = $("#productTbody").find("tr[id!=message]");
            if (tbody.length == 0) {
                alert("请选择要采购的商品");
                return false;
            }
            var suppContact = $("#suppContact").val();
            var suppPhone = $("#suppPhone").val();
            if (suppContact.trim() == "" || suppPhone.trim() == "") {
                alert("请完善采购信息");
                return false;
            }
            var texts = $("#productTbody").find("input[type=text]");
            for (var i = 0; i < texts.length; i++) {
                var sum = $(texts[i]).val();
                if (sum == 0) {
                    alert("好好填写价格   不能为000000000000   你家的东西不要钱啊!!!");
                    return false;
                }
            }
