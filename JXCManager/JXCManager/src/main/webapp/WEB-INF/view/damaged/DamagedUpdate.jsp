				...
				<div class="row-fluid sortable">
					<div class="box span12">
						...
						<div class="box-content">
							<form class="form-horizontal" action="damageRequestUpdate"
									...
									<table style="width: 80%; margin: 0px auto;">
										<tr>
											...
											<td><div class="control-group">
													...
													<div class="controls">
													<input type="hidden" name="damagedRequestId" value="${damagedRequest.damagedRequestId }" />
														<select id="requestEmp.empLoginName"
															...
															<c:forEach items="${employees}" var="s">
																<option value="${s.empLoginName}"
																	<c:if test="${s.empLoginName==damagedRequest.requestEmp.empLoginName }">selected="selected"</c:if>>${s.empLoginName}</option>
															</c:forEach>
														</select>
													</div>
												</div></td>
											...
											<td><div class="control-group">
													...
													<div class="controls">
														<select id="damagedStoreHouse"
															name="damagedStoreHouse.storeHouseId" disabled="disabled"
															data-rel="chosen">
															<option>请选择</option>
															<c:forEach items="${storehouses}" var="s">
																<option value="${s.storeHouseId }"
																	<c:if test="${s.storeHouseId==damagedRequest.damagedStoreHouse.storeHouseId }">selected="selected"</c:if>>${s.shName}</option>
															</c:forEach>
														</select>
													</div>
												</div></td>
												...
												<div class="control-group">
													...
													<div class="controls">
														...
														<textarea cols="15" rows="5" id="damagedReason"
															name="damagedReason" placeholder="请输入报损原因">${damagedRequest.damagedReason }</textarea>
													</div>
												</div>
											</td>
										</tr>
												...
												<div class="control-group">
													...
													<div class="controls">
														...
														<textarea cols="15" rows="5" id="remark" name="remark"
															placeholder="备注">${damagedRequest.remark}</textarea>
													</div>
												</div>
											</td>
										</tr>
									...
									<div class="box-content">
										<table
											...
											<tbody id="productTbody">
												<c:forEach items="${damagedRequestDetails}" var="s">
													<tr>
														...
														<td><a class="label label-important"
															...
															onclick="removeproductDetail(${s.dRDId},this)">移除</a></td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
	...
	<script type="text/javascript">
		$(function() {
			...
			$("#requestTime").val(
					parseInt(date.getMonth()) + 1 + "/" + date.getDate() + "/"
							+ date.getFullYear()); */
			var value='${damagedRequest.damagedStoreHouse.storeHouseId}';
		...
		function getStockPileByStoreHouse(storeHouseId,pageNo) {
			$.ajax({
				...
				data : "storeHouseId=" + storeHouseId+"&pageNo="+pageNo,
				dataType : "JSON",
			...
			for (var i = 0; i < toadd.length; i++) {
				...
				if ((toadd[i].flag !== undefined) == false) {
					productHtml += "<tr><td>"
							...
							+ "</td><td>"
							+ toadd[i].num
							+ "</td><td><a class=\"label label-important\" href=\"javascript:;\" id=\"removeproductDetail\" onclick=\"removeproductDetail(-1,this)\" >移除</a></td></tr>";
				}
			}
			//调用移除提示字样方法
			removeWhether();
		...
		//移除详情
		function removeproductDetail(no,a){
			if(no==-1){
				$(a).parent().parent().remove();
				whetherPro();
			}else{
				var s=confirm("确认移除吗?  此操作不可恢复");
				if(s==true){
					$(a).parent().parent().remove();
					whetherPro();
					$.ajax({
						type:"POST",
						url:"delDamagedRequestDetailById",
						data:"id="+no,
						dataType:"text",
						success:function(result){
							if(result=="\"1\""){
