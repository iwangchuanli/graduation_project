<html xmlns="http://www.w3.org/1999/xhtml" class="ks-webkit537 ks-webkit ks-chrome56 ks-chrome"><head>
    ...
    <script type="text/javascript">
            function resetAstro() {
                var astro = $("#astro");
                var monthOption =$("#J_Month");
                var dateOption = $("#J_Date");
                var monthSelectIndex = monthOption.get(0).selectedIndex;
                var dateSelectIndex = dateOption.get(0).selectedIndex;

                var month = parseInt(monthSelectIndex);
                var date = parseInt(dateSelectIndex);
                var s = "摩羯水瓶双鱼牡羊金牛双子巨蟹狮子处女天秤天蝎射手摩羯";
                var arr = [20, 19, 21, 21, 21, 22, 23, 23, 23, 23, 22, 22];
                
                var s = s.substr(month * 2 - (date < arr[month - 1] ? 2 : 0), 2);
                var ss = s+"座";
                   
                var obj = document.getElementById("astro");
                    obj.options[monthSelectIndex] =(new Option(s + "座", s + "座"));
                    obj.options[monthSelectIndex].selected = true;
                  
                
            }

            $(function(){
                $('#J_saveProfile').mousedown(function(){
                    var ss = $('#astro').val();
                    $('#hidastro').attr('value',ss);
                })
            })
...
<!-- end vmcommon/mytaobao -->
		...
		<div id="content">
	        <!-- 页面content内容开始 -->
          <link rel="stylesheet" href="./css/z_baseInfoSet_tbsp-sns-min.css">
<div id="content" class="layout grid-s160m0">
<link href="./css/app-config.css" type="text/css" rel="stylesheet">
<link rel="stylesheet" href="./css/z_baseInfoSet_tbmtb-profile0.0.23.0profilecsspf-base.css">
<link rel="stylesheet" href="./css/z_baseInfoSet_tbmtb-profile0.0.23.0profilecssmt-tips.css">
...
<div class="col-main">
  ...
  <ul>
    <li class="selected">
        <a href="base_info_set.htm"><span>个人资料</span></a>
    </li>
  </ul>
      <ul class="tab-sub">
      ...
      <li><a href="headset.htm?tracelog=Photo011"><span>头像照片</span></a></li>
            ...
            <div class="bd">
                <div class="sns-nf">
                <div class="tips-box">亲爱的<b>{{session::get('uname')}}</b>，填写真实的资料，有助于好友找到你哦。</div>
@if($info)
                <form id="baseInfoForm" name="baseInfoForm" method="post" class="infoForm" action="/baseInfoSet/store">
                {!! csrf_field() !!}
                    ...
                    <div id="main-profile" class="parts">
                        <p>
                            ...
                            <span class="pf-avatar-box">
                                <a class="pf-avatar">
                                    <img src="{{asset(session::get('head'))}}" style="height:80px;width:80px;">
                                                                    </a>
                                <a href="//i.taobao.com/user/headset.htm" class="pf-edit-avatar">编辑头像</a>
                                <input type="hidden" name="head" value="{{asset(Session::get('head'))}}">
                            </span>
                        </p>
                        <p>
                            <label>昵称：<em>*</em></label>
                            <input id="J_uniqueName-mask" class="f-txt" type="text" value="{{$info[0]['petname']}}" name="petname" maxlength="25">
                            <input id="J_uniqueName" name="_fm.b._0.u" type="hidden" value="" maxlength="25">
                        </p>
                        <p class="input-help-text">
                            <span>*昵称填写须知：与淘宝业务或卖家品牌冲突的昵称，淘宝将有可能收回</span>
                                                    ...
                                                    <p>
                            <label>真实姓名：</label>
                            <input id="J_realname-mask" class="f-txt" type="text" value="{{$info[0]['tname']}}" name="tname" maxlength="6">
                            <input id="J_realname" name="_fm.b._0.r" type="hidden" value="" maxlength="6">
                                                    </p>
                        <p>
                            <label>性别：<em>*</em></label>
                         @if($info[0]['sex'])
                            <label for="J_gender1" class="except"><input id="J_gender1" type="radio" name="sex" value="0">男</label>
                            <label for="J_gender2" class="except"><input id="J_gender2" type="radio" name="sex" checked value="1">女</label>
                        @else
                            <label for="J_gender1" class="except"><input id="J_gender1" type="radio" name="sex" checked value="0">男</label>
                            <label for="J_gender2" class="except"><input id="J_gender2" type="radio" name="sex" value="1">女</label>
                        @endif
                        </p>
                                                    <p>
                            <label>生日：</label>
                            <select id="J_Year" name="year">
                                <option selected="selected">年</option>
                                            <option value="{{$info[0]['year']}}" selected>{{$info[0]['year']}}</option>
            ...
            <select id="J_Month" name="month">
                <option selected="selected">月</option>
                                            <option value="{{$info[0]['month']}}" selected>{{$info[0]['month']}}</option>
            ...
            <select id="J_Date" name="day" onchange="resetAstro()">
                <option selected="selected">日</option>
                                            <option value="{{$info[0]['day']}}" selected>{{$info[0]['day']}}</option>
                                            <option value="1">1</option>
                                            <option value="2">2</option>
                                            <option value="3">3</option>
                                            <option value="4">4</option>
                            ...
                            <sub>（出生年份为保密）</sub>
                            <span>星座：</span>
                            <select id="astro"  name="astro" readonly>
                                <option value="{{$info[0]['astro']}}" selected>{{$info[0]['astro']}}</option>
                            </select>
                            <input type="hidden" name="hidastro" id="hidastro">
                        </p>
...
<form id="baseInfoForm" name="baseInfoForm" method="post" class="infoForm" action="/baseInfoSet/store">
                {!! csrf_field() !!}
                    ...
                    <div id="main-profile" class="parts">
                        ...
                        <p>
                            <label>昵称：<em>*</em></label>
                            <input id="J_uniqueName-mask" class="f-txt" type="text" value="" name="petname" maxlength="25">
                            <input id="J_uniqueName" name="_fm.b._0.u" type="hidden" value="" maxlength="25">
                        </p>
                        <p class="input-help-text">
                            <span>*昵称填写须知：与淘宝业务或卖家品牌冲突的昵称，淘宝将有可能收回</span>
                                                    ...
                                                    <p>
                            <label>真实姓名：</label>
                            <input id="J_realname-mask" class="f-txt" type="text" value="" name="tname" maxlength="6">
                            <input id="J_realname" name="_fm.b._0.r" type="hidden" value="" maxlength="6">
                                                    </p>
                        <p>
                            <label>性别：<em>*</em></label>
